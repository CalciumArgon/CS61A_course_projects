...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        !!! exit 和 entrance 是其他 Place 类
        初始化一个 Place 时, 不会处理它的 entrance
        而是会把它 exit 位置的 entrance 设置为自己
        """
        self.name = name
        self.exit = exit
    ...
    def add_insect(self, insect):
        ...
        """
        # if insect.__class__ == QueenAnt and not insect.trueQueen:
        #     return
        insect.add_to(self)
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        # print("Place.remove_insect is called -> insect.remove_from")
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self    # place 里记录该 ant
            self.place = place
        else:   # 位置不空, 开始比较两只 ant 的种类
            # BEGIN Problem 8
            # print("Already have ant")
            origin = place.ant
            if self.is_container() and origin.__class__ is QueenAnt and origin.trueQueen == True:
                # 保护 Queen 时由于无法移走 Queen 只能特判
                place.ant = self
                self.place = place
                self.contained_ant = origin
            elif self.is_container() and not origin.is_container():
                origin.remove_from(place) # 先移走 origin
                self.add_to(place)
                self.contained_ant = origin # 再正常调用 add_to
                origin.place = place
            elif not self.is_container() and origin.is_container() and origin.can_contain(self):
                origin.contained_ant = self
                self.place = place
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        # 遍历所有位置, 寻找所有位置上是否有 bee, 有就直接返回 (已经最近了)
        # 直到 Place == beehive 停止寻找
        if self.name == 'Short':
            self.max_range = 3
        elif self.name == 'Long':
            self.min_range = 5
        ...
        in_front = self.place
        for _ in range(self.min_range):     # 先初始化到最小距离
            in_front = in_front.entrance
            if in_front == None or in_front == beehive:
                return None
        
        if in_front == None or in_front == beehive: # min_range == 0 时没法检查 self.place 情况
            return None

        current = self.min_range    # 记录查找到的范围
        while current <= self.max_range and in_front != beehive:
            if in_front.bees != []:
                return bee_selector(in_front.bees)
            in_front = in_front.entrance
            current += 1
        return None
        # END Problem 3 and 4
    ...
    def throw_at(self, target:Insect):
        ...
        # print(f"Begin throwing at {target}")
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        for bee in self.place.bees[:]:  # 防止有 bee 死掉后该列表长度变化
            bee.reduce_health(amount)
        alive_bees = self.place.bees[:]

        Ant.reduce_health(self, amount)
        if self.place == None:  # Ant 死掉 额外伤害
            for bee in alive_bees:  # 死掉之后 Ant 和该位置就无关联了
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    name = "Hungry"
    chew_duration = 3   # 消化时间

    def __init__(self, health=1):
        super().__init__(health)
        self.chewing = 0    # 记录当前需要消化的时间
    
    def action(self, gamestate):
        # self.place 存在 Ant 类里
        if self.place.bees != [] and self.chewing == 0:
            choose_one_eat = bee_selector(self.place.bees)
            choose_one_eat.reduce_health(choose_one_eat.health)
            self.chewing = self.chew_duration   # 按统一标准重新计时消化时间
        elif self.chewing != 0:
            self.chewing -= 1
...
class ContainerAnt(Ant):
    """
    注意有两种给 ant 加上保护的方法:
    1. 调用 Some_container.contain_ant(some_ant)
      相当于直接改 self.contained_ant 参数
    2. 将某 ant 直接 add_to(place), 如果该位置有 ant 且其中一个是 container
      就会自动完成覆盖和包含, 调用的是 Ant(Insect) 类里的 add_to(place)
    """

    def __init__(self, health=2):
        Ant.__init__(self, health=health)
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if other.__class__ != ContainerAnt and self.contained_ant == None:
            return True
        return False
        # END Problem 8
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant != None:
            # print("Have done inside action here.")
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self, health=2):
        ContainerAnt.__init__(self, health=health)
        self.contained_ant = None
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, health=1):
        # BEGIN Problem EC
        ScubaThrower.__init__(self, health=health)
        self.health = 1
        if QueenAnt.existQueen:     # 如果已经存在一个, 则将此 Queen 血量减至 0
            self.health -= 1
            self.trueQueen = False
        else:
            QueenAnt.existQueen = True
            self.trueQueen = True
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        if not self.trueQueen:  # 如果是假的 Queen, 在攻击后会被 death_callback() 回收
            Ant.death_callback(self)
            return
        ThrowerAnt.action(self, gamestate=gamestate)    # 自己攻击
        point = self.place.exit
        while point != None:
            # 将 point 处的所有 Ant 执行 buff()
            if point.ant != None:
                point.ant.buff()
                if point.ant.is_container() and point.ant.contained_ant != None:
                    point.ant.contained_ant.buff()
            point = point.exit
        # END Problem EC
...
class GameState:
    ...
    def remove_ant(self, place_name):
        ...
        # print(f"Now begin to remove ant from {place}")
        if place.ant is not None:
            # print(f"{place.ant} is in here")
